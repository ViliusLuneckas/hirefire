    ...
    class Base
      ...
      def fire
        if jobs == 0 and workers > min_workers
          Logger.info("All queued jobs have been processed. " + (min_workers > 0 ? "Setting workers to #{min_workers}." : "Firing all workers."))
          workers(min_workers)
      ...
      def log_and_hire(amount)
        Logger.info("Hiring more workers so we have #{ amount } in total.")
        workers(amount)
      end
